...
//CreateTree::Instance() -> fibresParameters -> Fill(fibre_radius,fibre_distance);
  ...
  module_z = fibre_length + Second_fibre_length;
  First_module_z = fibre_length;
  PLEX_depth = 15;
 ...
 Wires_dist = 50;
  ...
  expHall_z = (First_module_z + Second_module_z ) * 4 + lp_dist + lp_depth ;
...
G4VPhysicalVolume* DetectorConstruction::Construct ()
{
  ...
  // The calorimeter
  // ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
  G4VSolid * calorimeterS = new G4Box ("calorimeterS", 0.5 * module_xy, 0.5 * module_yx, 0.5 * (First_module_z + Second_module_z)) ;
  ...
  {
    new G4PVPlacement (0, G4ThreeVector (0., 0., 0.), calorimeterLV, "calorimeterPV", worldLV, false, 0, checkOverlaps) ;
    new G4PVPlacement (0, G4ThreeVector (0., 0., 1.*(First_module_z + Second_module_z)), calorimeterLV, "postshower1PV", worldLV, false, 0, checkOverlaps) ;
    new G4PVPlacement (0, G4ThreeVector (0., 0., 2.*(First_module_z + Second_module_z)), calorimeterLV, "postshower2PV", worldLV, false, 0, checkOverlaps) ;
  }
...
G4VSolid * absorber1S = new G4Box ("absorber1S", 0.5 * module_xy, 0.5 * (module_yx), 0.5 * First_module_z) ;
   G4LogicalVolume * absorber1LV;
   absorber1LV = new G4LogicalVolume (absorber1S, AbMaterial, "absorber1LV") ;
  new G4PVPlacement (0, G4ThreeVector (0., 0., -0.5*(First_module_z+Second_module_z) + 0.5* First_module_z), absorber1LV, "absorber1PV", calorimeterLV, false, 0, checkOverlaps) ;
  ...
  new G4PVPlacement (0, G4ThreeVector (0., 0., -0.5*(First_module_z+Second_module_z) + First_module_z + 0.5*Second_module_z), absorber2LV, "absorber2PV", calorimeterLV, false, 0, checkOverlaps) ;
      ...
      else
{
  ...
  for (float x = -0.5*module_xy+startX; countX <nFibresAlongX; x += fibreDistanceAlongX)
  {
    ...
    for (float y = - 0.5 * module_yx + startY; countY < nFibresAlongY; y += fibreDistanceAlongY)
    {
      ...
      // **************  ***************
      // # Above
     if (x >= -1.5*fibres_x1 && x<= 1.5*fibres_x1 && y >= 0.5*fibres_y1 && y <= 1.5*fibres_y1)
	{
      name = Form("holePV %d",index);
      new G4PVPlacement (0,G4ThreeVector(x_c,y_c,0.), holeLV, name, absorber1LV, false, 0, checkOverlaps) ;
 ...
 int indexion3;
  for (float x = -0.5*Second_module_xy+Second_startX; countX3 < Second_nFibresAlongX; x += Second_fibreDistanceAlongX)
  {
    ...
    for (float y = - 0.5 * Second_module_yx + Second_startY; countY3 < Second_nFibresAlongY; y += Second_fibreDistanceAlongY)
    {
      ...
      // **************  ***************
      if (x >= -1.5*Second_fibres_x1 && x<= 1.5*Second_fibres_x1 && y >= 0.5*Second_fibres_y1 && y <= 1.5*Second_fibres_y1)
{      name = Form("hole2PV %d",index);
      new G4PVPlacement (0,G4ThreeVector(x_c,y_c,0.), hole2LV, name, absorber2LV, false, 0, checkOverlaps) ;
 ...
 else
{
 ...
 new G4PVPlacement (0,G4ThreeVector(x_c,y_c,0.), fibre23SLV, name, absorber2LV, false, 0, checkOverlaps) ;
  ...
  if( !postshower )
  {
    ...
    new G4PVPlacement (0, G4ThreeVector (0., 0., 0.5*First_module_z+0.5*depth+0.5*Second_module_z),          gapLayerLV, "gapLayerPV", worldLV, false, 0, checkOverlaps) ;
    new G4PVPlacement (0, G4ThreeVector (0., 0., 0.5*First_module_z+0.5*Second_module_z+depth+0.5*(gap_l-depth)), gapLV,      "gapPV", worldLV, false, 0, checkOverlaps) ;
  }
  ...
  if( !postshower )
  {
    ...
    new G4PVPlacement (0, G4ThreeVector (0., 0., 0.5*First_module_z + 0.5*Second_module_z +gap_l+0.5*depth),          detLayerLV, "detLayerPV", worldLV, false, 0, checkOverlaps) ;
    new G4PVPlacement (0, G4ThreeVector (0., 0., 0.5*First_module_z+0.5*Second_module_z+gap_l+depth+0.5*(det_l-depth)), detLV,      "detPV", worldLV, false, 0, checkOverlaps) ;
  }
