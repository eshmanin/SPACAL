...
ADD ${CLHEP_URL} /tmp/
RUN yum -y update && yum clean all && yum -y install sudo which gcc gcc-c++ git wget cmake make libXft  libXpm libSM libXext mesa-libGLU.1686 libGLU libXmu &&\
    rpm -Uvh /tmp/${CLHEP}
...
RUN tar xf /tmp/${GEANT4}               -C /usr/share/ && rm /tmp/${GEANT4} && mkdir /usr/share/Geant4-10.5.1-Linux/data && \
    ...
    tar xf /tmp/root_v6.18.04.Linux-centos7-x86_64-gcc4.8.tar.gz -C /usr/share/ && source /usr/share/root/bin/thisroot.sh
